...
public static int getOrientation(Context context, Uri photoUri) {
    ...
    try {
        // Query the MediaStore for the orientation column
        ...
        if (cursor != null && cursor.getCount() > 0) {
            cursor.moveToFirst();
            orientation = cursor.getInt(0); // Get the orientation
        } else {
            Log.e("Image", "No orientation data available for the image");
        }
    } catch (Exception e) {
        Log.e("Image", "Error querying orientation: " + e.getMessage());
        e.printStackTrace();
    } finally {
    ...
    return orientation; // Return the orientation, -1 if not found
}
